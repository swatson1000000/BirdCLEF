"""Merge 3 Perch-extraction partitions into a single dataset staging dir.

Reads:  /tmp/perch_extract_v{6,7,8}/full_train_audio_{perch,meta}_p{0,1,2}of3.{npz,parquet}
Writes: kaggle_datasets/train-audio-perch/{full_train_audio_perch.npz,
                                            full_train_audio_meta.parquet,
                                            dataset-metadata.json}

Verifies shape, dtype, row-count alignment, and unique stem count ≥34,000.
...
PARTITION_COUNT = 3
PART_DIRS = [Path(f"/tmp/perch_extract_v{6+i}") for i in range(PARTITION_COUNT)]
STAGE = Path("/home/swatson/work/MachineLearning/kaggle/BirdCLEF/kaggle_datasets/train-audio-perch")
...
def main() -> int:
    ...
    for part_id, d in enumerate(PART_DIRS):
        npz_path = d / f"full_train_audio_perch_p{part_id}of{PARTITION_COUNT}.npz"
        meta_path = d / f"full_train_audio_meta_p{part_id}of{PARTITION_COUNT}.parquet"
    ...
    n_stems = meta_full["stem"].nunique()
    print(f"unique stems: {n_stems}")
    if n_stems < 34000:
        print(f"STEM COUNT BELOW GATE: {n_stems} < 34000", file=sys.stderr)
        return 6

    out_npz = STAGE / "full_train_audio_perch.npz"
    out_meta = STAGE / "full_train_audio_meta.parquet"
    print(f"\nwriting {out_npz}")
    ...
    dataset_meta = {
        "title": "BirdCLEF 2026 train_audio Perch features",
        "id": "stevewatson999/birdclef-2026-train-audio-perch",
