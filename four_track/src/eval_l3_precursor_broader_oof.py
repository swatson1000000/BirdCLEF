"""Generate L3-precursor (CE + seed123 + no-mixstyle) 5-fold broader-pool OOF.

Recipe (b) of the L3 multi-recipe SED bag. Runs each of the 5 seed123_ce
fold ckpts on the 1478-segment labeled soundscape val pool (built via
`train_a1.build_soundscape_val`, identical window selection to A2/v4 OOF
so the comparison is apples-to-apples). Stacks per-fold probs, takes
sig-mean across folds, reports macro AUC on present classes vs the
A2 anchor (0.8402).

Output: four_track/data/l3_precursor_broader_oof.npz with keys
...
CKPT_NAME_FMT = "a1_tf_efficientnet_b0.ns_jft_in1k_fold{f}_seed123_ce.pt"
OUT_PATH = FT_ROOT / "data" / "l3_precursor_broader_oof.npz"

BATCH_SIZE = 32
N_FOLDS = 5

A2_ANCHOR_AUC = 0.8402  # A2 5-fold sig-mean broader-pool ensemble
GATE_DELTA = 0.05  # broader-pool OOF must beat A2 by ≥+0.05 to justify a fresh LB slot standalone
...
def main() -> int:
    ...
    print("", flush=True)
    print("=" * 60, flush=True)
    print("L3-precursor (CE+seed123+no-mixstyle) broader-pool 5-fold OOF", flush=True)
    print("=" * 60, flush=True)
    for f in range(N_FOLDS):
    ...
    delta = ensemble_auc - A2_ANCHOR_AUC
    print(f"  delta vs A2:  {delta:+.4f}", flush=True)
    if ensemble_auc < A2_ANCHOR_AUC:
        verdict = "STANDALONE REGRESSION vs A2"
    elif ensemble_auc < GATE_AUC:
        verdict = "STANDALONE below +0.05 gate — bag delta is the real gate"
    else:
        verdict = "STANDALONE GATE-PASS (rare; check before claiming a slot)"
    print(f"  standalone verdict: {verdict}", flush=True)
    print("=" * 60, flush=True)
    print("  (real L3-prec gate is the A2 ⊕ L3-prec bag delta — see plan §663–667)", flush=True)
