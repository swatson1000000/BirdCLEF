"""A2-1: Generate A1 5-fold pseudo-labels on the 10592 unlabeled train_soundscapes.
...
- Forward through 5 A1 JIT folds, sigmoid-mean across folds (per Probe A §25.13)

Output: data/processed/a1_pseudo_soundscape.npz with:
...
Usage:
  python -u src/a2_pseudo_label_a1.py [--device cuda|cpu] [--limit N]
"""
...
CKPT_DIR = ROOT / 'kaggle_datasets' / 'a1-effb0-ckpts'
...
def main():
    ...
    ap.add_argument('--out', default=str(OUT_DIR / 'a1_pseudo_soundscape.npz'))
    args = ap.parse_args()
