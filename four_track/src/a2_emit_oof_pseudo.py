"""src/a2_emit_oof_pseudo.py — Track A2 step 1: OOF pseudo-label emission.

Predict per-(file, 5-sec-window) sigmoid probs on every train_soundscapes
file using a 5-fold OOF assignment over the A1 EffNet-B0 ckpts.

OOF rule: each soundscape file is assigned a bucket k in [0,4] via a stable
hash of its filename. Predictions for files in bucket k are averaged across
the 4 A1 ckpts trained on folds != k. This avoids val leakage when the
emitted pseudo-labels are reused in Track A2 retraining.

Source ckpts: models/a1/a1_..._fold{F}_seed42_hybrid.pt — the P8 5-fold raw
state_dicts (LB 0.928 single-ensemble vs v62 0.931, within noise floor; OOF
ensemble averages 4 of these so individual-ckpt quality is absorbed).

Output: data/processed/a2_train_ss_oof_probs.npz with keys
...
CKPT_TEMPLATE    = "a1_tf_efficientnet_b0.ns_jft_in1k_fold{f}_seed42_hybrid.pt"
SS_DIR           = RAW / "train_soundscapes"
OUT_PATH         = FT_ROOT / "data" / "processed" / "a2_train_ss_oof_probs.npz"
